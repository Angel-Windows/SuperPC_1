...
public class CE extends Sim_entity {
    private Sim_port in1, in2, in5, in6, in7;
    ...
    CE(String name, double delay) {
        ...
        add_port(in1);
        add_port(in2);
        add_port(in5); add_port(in6); add_port(in7);
    ...
    public void body() {
        while (Sim_system.running()) {
            ...
            if (event.from_port(in1)) {
                ...
                // Равномерное распределение задач по узлам с учетом их производительности
                switch (taskCounter % 3) {
                    case 0:
            ...
            if (event.from_port(in5) || event.from_port(in6) || event.from_port(in7)) {
                link = true;
